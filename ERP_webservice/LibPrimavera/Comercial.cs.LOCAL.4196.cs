...
namespace FirstREST.Lib_Primavera
{
    public class Comercial
    {
        ...
        public static List<Model.Utilizador> ListaVendedores()
        {
            ...
            List<Model.Utilizador> listVendedores = new List<Model.Utilizador>();
            if (PriEngine.Platform.Inicializada)
            {
                //if (PriEngine.InitializeCompany("BELAFLOR", "admin", "admin") == true){
                String query = "SELECT * FROM PRIEMPRE.dbo.Utilizadores WHERE PerfilSugerido = 'Comercial I' AND activo = '1' ";
                ...
                while (!objList.NoFim())
                {
                    seller = new Model.Utilizador();
                    seller.Cod = objList.Valor("Codigo");
                    seller.Tipo = 2;
                    seller.Nome = objList.Valor("Nome");
                    seller.Email = objList.Valor("Email");
        ...
        public static Lib_Primavera.Model.Utilizador GetVendedor(string codVendedor)
        {
            ...
            if (PriEngine.Platform.Inicializada)
            {
                ...
                objVen = PriEngine.Engine.Consulta(query);
                if (objVen.NumLinhas() > 0)
                {
                    seller.Cod = objVen.Valor("Codigo");
                    seller.Tipo = 2;
                    seller.Nome = objVen.Valor("Nome");
                    seller.Email = objVen.Valor("Email");
                    seller.Telefone = objVen.Valor("Telemovel");
                    seller.Idioma = objVen.Valor("Idioma");
                    return seller;
                }
                else
                {
        ...
        public static List<Model.Utilizador> ListaAdmins()
        {
            ErpBS objMotor = new ErpBS();
            //MotorPrimavera mp = new MotorPrimavera();
            StdBELista objList;

            Model.Utilizador adm = new Model.Utilizador();
            List<Model.Utilizador> listAdmins = new List<Model.Utilizador>();
            if (PriEngine.Platform.Inicializada)
            {

                //if (PriEngine.InitializeCompany("BELAFLOR", "admin", "admin") == true){
                String query = "SELECT * FROM PRIEMPRE.dbo.Utilizadores WHERE Administrador = '1' AND activo = '1'";
                objList = PriEngine.Engine.Consulta(query);
                while (!objList.NoFim())
                {
                    adm = new Model.Utilizador();
                    adm.Cod = objList.Valor("Codigo");
                    adm.Tipo = 1;
